...
The catalogue holds **490 keys** in two locales. `src/i18n/en.ts` is the reference — its text is
...
| `skyscraper.*` | 21      | `src/game/skyscraper.ts`, whose getters read all twenty-one; the card they end up on is `src/widgets/level-briefing/ui/level-briefing.ts`, which is handed the finished strings and so names no key itself                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
### `documentation.html` — the reference page, 87 `docs.*` keys
...
### `src/game/skyscraper.ts` — 21 `skyscraper.*` keys

The Skyscraper block: levels built on how real lift systems are actually dispatched. Three keys per
...
particular mistake; a level here names an idea and hands over the building.

Every key is read by a getter on the level table, never by a widget. `src/widgets/level-briefing` is
handed the finished `title` and `briefing` strings by `src/pages/game/index.ts`, and
`src/ui/editor.ts` is handed the finished `startingCode`, so none of the three names a key. That is
what keeps the card localised despite it: the getters render at the moment they are read, and the
page reads them on every redraw, so a language change composes them again in the new language.
...
| Key                                 | English                                | Notes                                                                                                                                    |
| ----------------------------------- | -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `skyscraper.sky1.title`             | Eleven floors for one passenger        | the level's name, on the briefing card and nowhere else; the switcher's 118px trigger says "Tower 1" instead                             |
| `skyscraper.sky1.briefing.html`     | Twelve floors, three cars, and a run…  | markup; carries `<em>` and `<span class="emphasis-color">` around the terms it introduces. Quotes no threshold — the goal bar owns those |
...
| `skyscraper.sky2.title`             | Everyone starts in the lobby           | the morning up-peak, shown small                                                                                                         |
| `skyscraper.sky2.briefing.html`     | Ten floors, two cars, and a building…  | markup; `<em>` around the term the level introduces, the morning up-peak                                                                 |
...
| `skyscraper.sky3.title`             | Sixteen floors before nine             | the same morning at scale, and the block's first level with medals                                                                       |
| `skyscraper.sky3.briefing.html`     | Sixteen floors, four cars with room…   | markup carried by none of it: this level introduces no term, `sky2` did                                                                  |
...
| `skyscraper.sky4.title`             | Now the lobby is the destination       | the evening down-peak, shown small                                                                                                       |
| `skyscraper.sky4.briefing.html`     | Twelve floors, two cars, and the same… | markup; `<em>` around the term the level introduces, the evening down-peak                                                               |
...
| `skyscraper.sky5.title`             | Fourteen floors, all going down        | the evening at scale; the one level judged on the clock and the longest wait                                                             |
| `skyscraper.sky5.briefing.html`     | Fourteen floors, three cars, and…      | no markup either; names the two figures the run is judged on, quotes neither                                                             |
| `skyscraper.sky5.startingCode.code` | (the sorted sweep)                     | `.code`; `sky3`'s program unchanged, comments included                                                                                   |
| `skyscraper.sky6.title`             | Lunch runs both ways                   | midday traffic, shown small                                                                                                              |
| `skyscraper.sky6.briefing.html`     | Nine floors at midday. Lunch traffic…  | markup; `<em>` around the term the level introduces, lunch traffic                                                                       |
...
| `skyscraper.sky7.title`             | Neither half of the trip is free       | midday at scale, and the level the block is built towards                                                                                |
| `skyscraper.sky7.briefing.html`     | Twelve floors, three cars, and the…    | markup; `<em>` around moves, the axis this run is scored on                                                                              |
...
### `src/ui/completions.ts` — 34 `completion.*` keys
