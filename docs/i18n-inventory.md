...
The catalogue holds **367 keys** in two locales. `src/i18n/en.ts` is the reference — its text is
...
| `page.hint.html`                | In the editor: your code is saved as you type. `<kbd data-mod-key>`Ctrl`</kbd>`+`<kbd>`Enter`</kbd>` starts the challenge over with it. … | markup; `localisePage` calls `labelModifierKeys` last, having just overwritten with `innerHTML` the `<kbd>` labels it fixes                         |
...
| `docs.play.shortcuts.html`                          | Inside the editor, Ctrl+Enter starts the challenge again with what you have written …                           | markup; same `data-mod-key` caveat as `page.hint.html`                                                                             |
...
see: the two sit on adjacent lines of a 367-key file and nothing in the file marks them as a
...
Built and unit-tested, not yet wired into `src/app/app.ts` or `index.html` — `presentThemeSwitch`
...
| `src/shared/lib/dom.ts` `requireElement`; `src/ui/templates.ts` `renderElement`; `src/ui/presenters.ts` `renderUser` | `Missing required element`, `Expected markup describing exactly one element`, `Expected the user template to render an SVG element` | Invariants. If a player ever reads one, the bug is that it was thrown, not that it was in English.                                                                                                                                                       |
