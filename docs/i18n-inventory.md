...
other twenty-two are the statistics panel's — a caption and a tooltip for each of its eleven
`page.stats.*` tiles — written at runtime by `src/widgets/stats-panel/ui/stats-panel.ts` into the
region the shell leaves empty, and kept here because they are one prefix and read as one table.
...
| `page.stats.transportedTitle`       | How many passengers have reached the floor they asked for, so somebody still riding is not counted…            | a `title` attribute on the same tile as `page.stats.transported`                                                                                                      |
| `page.stats.elapsedTime`            | Elapsed time                                                                                                   |                                                                                                                                                                       |
| `page.stats.elapsedTimeTitle`       | The run's own clock, which the speed control makes pass faster or slower than real time, and which…            | a `title` attribute on the same tile as `page.stats.elapsedTime`; says whose clock it is, since the speed control moves it                                            |
...
| `page.stats.stopsTitle`             | One stop is counted each time a car comes to rest at a floor and opens its doors, so a car sent to the…        | a `title` attribute on the same tile as `page.stats.stops`; text of `docs.play.statistics.html` word for word                                                         |
...
| `page.stats.peoplePerStopTitle`     | Everyone who got in or out, over the stops counted above, so opening the doors where nobody is waiting…        | a `title` attribute on the same tile as `page.stats.peoplePerStop`; text of `docs.play.statistics.html` word for word                                                 |
...
| `page.stats.avgLoadTitle`           | How full the cars were, averaged over the moves counted above, so a car standing still is not in the…          | a `title` attribute on the same tile as `page.stats.avgLoad`; text of `docs.play.statistics.html` word for word                                                       |
...
ones `presentStats` and `goal-bar.ts`'s own meters already show, and their tooltips with them — so
this prefix holds only the two figures with no production precedent, a caption and a tooltip
apiece, plus the disclosure summary that reveals the panel's nine secondary tiles. Every tile has a
tooltip: the field carrying its key is required, so a fourteenth tile cannot arrive unexplained.
...
| `game.statsPanel.waitingNowTitle` | How many passengers are standing on a floor at this moment with no car yet carrying them | a `title` attribute on the same tile as `game.statsPanel.waitingNow`                                       |
| `game.statsPanel.aboardNow`       | Riding now                                                                               | sums every elevator's occupied `userSlots`, the same test `elevatorSnapshot` uses                          |
| `game.statsPanel.aboardNowTitle`  | How many passengers are inside a car at this moment, on their way                        | a `title` attribute on the same tile as `game.statsPanel.aboardNow`                                        |
