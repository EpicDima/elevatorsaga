...
### `src/game/skyscraper.ts` — 19 `skyscraper.*` keys
...
which is `sky2` for traffic profiles, `sky8` for zoning and `sky11` for destination dispatch. The
...
That is why `sky2`'s paragraph ends by naming the evening peak and lunch traffic it does not itself
play: it is the only place levels 3 to 7 are introduced at all, exactly as `sky8`'s is for 9 and 10
and `sky11`'s is for 12 and 13.
...
| `skyscraper.sky2.title`              | Everyone starts in the lobby          | the level's name, on the briefing card and nowhere else; the switcher's 118px trigger says "Level 21" instead                                                 |
...
| `skyscraper.sky8.title`              | Not every car goes everywhere         | the level's name, on the second of the block's three briefing cards                                                                                           |
...
| `skyscraper.sky11.title`             | Nobody presses up or down             | the level's name, on the third and last briefing card of the block                                                                                            |
| `skyscraper.sky11.briefing.html`     | The hall buttons are gone. Instead o… | markup; `<em>` around _destination dispatch_, `<code>` around the event, the method that books a car, and the two ways a car is sent, the second in two calls |
| `skyscraper.sky11.startingCode.code` | (the booking rotation)                | `.code`; books a car for every journey and sends none, which is the mistake the card's last sentence names                                                    |
| `skyscraper.sky12.startingCode.code` | (the booking rotation that sends)     | `.code`; `sky11`'s program with the sending added, the `//` comment moved onto whose turn it is                                                               |
| `skyscraper.sky13.startingCode.code` | (the nearest car with room)           | `.code`; the longest starter in the block — a distance-sorted queue and a load check — and both of its `//` comments run to two lines                         |
...
seed — `sky3`'s and `sky10`'s take silver, `sky5`'s, `sky9`'s, `sky12`'s and `sky13`'s bronze, and
the other seven lose. `src/game/skyscraper.ts` records the figures that bracket each of them and
...
| `game.levelSwitcher.chapterBlockLabel`       | Chapter {number}                | takes `{number}`; the caption over each block of levels — chapter one is the numbered levels, chapter two the Skyscraper block that carries on their numbering                                             |
