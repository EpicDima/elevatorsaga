...
The catalogue holds **464 keys** in two locales. `src/i18n/en.ts` is the reference — its text is
...
| `docs.play.shortcuts.html`                          | Inside the editor, Ctrl+Enter starts the challenge again with what you have written …                           | markup; `src/docs.ts` calls `labelModifierKeys` on the page this fills, which is what relabels its two `<kbd data-mod-key>`s       |
...
| `tutorial.finish.message`           | Eight tasks, and the last of them was level 1: the same three floors, the same elevator, the same fifteen… |                                                                                                                                                 |
| `tutorial.finish.nextTask`          | Next task                                                                                                  | not `game.feedback.next`, which says "Next level" — see the note below                                                                          |
...
### `src/widgets/level-switcher/ui/level-switcher.ts` — 7 `game.levelSwitcher.*` keys
...
the third block's caption and the tile inside it, the two step buttons, and the two tile labels
the level list has no counterpart for, since it never names a learning-track task or a locked
level.
...
| `game.levelSwitcher.otherBlockLabel`          | Other                             | the third block's caption — everything that is neither a lesson nor a numbered level, which today is free play alone |
