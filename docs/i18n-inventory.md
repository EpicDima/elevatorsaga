...
The catalog holds **502 keys** in two locales, each locale spread over two files. `src/i18n/en.ts`
...
```sh
grep -hoE '^  "[^"]+"' src/i18n/en.ts src/i18n/docs-en.ts | wc -l       # 502
...
four block captions reuses `game.level.nav.label`, and a level
