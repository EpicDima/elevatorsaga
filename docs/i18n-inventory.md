...
| `chapter2.level2.title`              | Everyone starts in the lobby          | the level's name, on the briefing card and nowhere else; the switcher's 118px trigger says "Level 21" instead                                                 |
...
### `src/ui/templates.ts` — 14 `game.*` keys
...
The app bar's level-switcher popover, and its step buttons either side of the trigger. Its two
blocks of levels are chapters of one count — chapter two carries on chapter one's numbering — so an
unmedalled tile of either, and the trigger under either, reuse `game.level.nav.link`. This prefix
only holds what that cannot say: the block captions, the two step buttons, the lesson tiles and
their trigger, the sandbox tile, and the name a tile takes once a medal is on record. The medal
names themselves come from `game.goalBar.tier.*`, through `TIER_NAME_KEY` in
`src/entities/level-tier/ui/tier-name.ts`, which this widget and the goal bar share so that one
medal is called one word.
...
| `game.levelSwitcher.chapterBlockLabel`       | Chapter {number}                | takes `{number}`; the caption over each block of levels — chapter one is the numbered levels, chapter two chapter two that carries on their numbering                                                      |
...
| `game.levelSwitcher.levelTileEarnedLabel`    | Level {number}, {tier}          | takes `{number}` and `{tier}`; accessible name of a level tile in either chapter once a medal is on record. The badge's stars are `aria-hidden`, so this is the only place a screen reader hears the medal |
