...
The catalog holds **511 keys** in two locales, each locale spread over two files. `src/i18n/en.ts`
...
```sh
grep -hoE '^  "[^"]+"' src/i18n/en.ts src/i18n/docs-en.ts | wc -l       # 511
...
| `game.levelSwitcher.tutorialBlockLabel`        | Learning track                    | the caption over the block of lessons; the panel around a lesson is named after the level it teaches, so this and the tiles under it are where the track is named at all                          |
