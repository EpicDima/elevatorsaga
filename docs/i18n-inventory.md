...
| `game.levelSwitcher.challengeTileLockedLabel` | Level {number}, locked            | takes `{number}`; accessible name of a locked, non-demo challenge tile, drawn as a `<button disabled>` rather than an `<a>` |
