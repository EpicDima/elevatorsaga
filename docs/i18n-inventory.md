...
The catalogue holds **463 keys** in two locales. `src/i18n/en.ts` is the reference — its text is
...
three block captions reuse `tutorial.panel.label` and `game.level.nav.label`, and a level
...
level list has no counterpart for — it never names a learning-track level — and the trigger's own
name for a lesson.
