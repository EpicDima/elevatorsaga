...
The catalog holds **521 keys** in two locales, each locale spread over two files. `src/i18n/en.ts`
...
```sh
grep -hoE '^  "[^"]+"' src/i18n/en.ts src/i18n/docs-en.ts | wc -l       # 521
...
### `src/ui/templates.ts` — 15 `game.*` keys
...
### `src/widgets/level-switcher/ui/level-switcher.ts` — 13 `game.levelSwitcher.*` keys

The app bar's level-switcher popover, and its step buttons either side of the trigger. One of its
four block captions reuses `game.level.nav.label`, and an unmedalled level
tile reuses `game.level.nav.link` — this prefix only holds what those cannot say:
the other three blocks' captions and the tiles inside them, the two step buttons, the tile labels the
level list has no counterpart for — it never names a learning-track or Skyscraper level, nor a medal
— and the trigger's own names for those two. The medal names themselves come from
`game.goalBar.tier.*`, through `TIER_NAME_KEY` in `src/entities/level-tier/ui/tier-name.ts`, which
this widget and the goal bar share so that one medal is called one word.
...
| `game.levelSwitcher.skyscraperTileLabel`       | Skyscraper level {number}         | takes `{number}`; accessible name of a Skyscraper tile with no medal on record                                                                                                                    |
...
| `game.levelSwitcher.levelTileEarnedLabel`      | Level {number}, {tier}            | takes `{number}` and `{tier}`; accessible name of a numbered level tile once a medal is on record. The badge's stars are `aria-hidden`, so this is the only place a screen reader hears the medal |
| `game.levelSwitcher.skyscraperTileEarnedLabel` | Skyscraper level {number}, {tier} | takes `{number}` and `{tier}`; the same for a Skyscraper tile                                                                                                                                     |
