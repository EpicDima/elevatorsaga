...
### `src/features/hotkeys-help/ui/hotkeys-modal.ts` — 19 `game.hotkeys.*` keys

The `hotkeys-help` feature's own dialog is `<dialog class="keys">`: a title, a close button and two
headed groups of rows pairing a hotkey with what it does. The split is not cosmetic — every
shortcut in the first group is dropped while the cursor is in the code editor, which is why each
heading names its own scope rather than leaving the rows to imply one. Mod- bindings are spelled as
`<kbd>`s joined by `+`, the convention `docs.play.shortcuts.html` already uses;
`src/ui/shortcuts.ts`'s `labelModifierKeys` resolves each one per visitor at runtime, so nothing
here needs a static paragraph explaining that Windows and Linux read `Ctrl` for `⌘`. Whoever mounts
the dialog live still has to call `labelModifierKeys` against it themselves, the same way
...
| `game.hotkeys.outsideEditor`   | Outside the code editor                  | the first group's heading, over the page's own shortcuts                        |
...
| `game.hotkeys.editorOnly`      | In the code editor                       | the second group's heading, over CodeMirror's own bindings                      |
