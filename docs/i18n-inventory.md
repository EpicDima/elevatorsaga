...
The catalog holds **543 keys** in two locales, each locale spread over two files. `src/i18n/en.ts`
...
```sh
grep -hoE '^  "[^"]+"' src/i18n/en.ts src/i18n/docs-en.ts | wc -l       # 543
...
| `page.*`       | 32      | `index.html`, through `data-i18n` and `data-i18n-attr` (nine of them; `page.noscript` excepted, see below), `src/widgets/stats-panel/ui/stats-panel.ts` (22, every `page.stats.*` but the panel's own `aria-label`), `src/widgets/app-bar/ui/settings-menu.ts` (`page.language.label`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
### `index.html` and the statistics panel — 32 `page.*` keys
...
Nine of the keys below are the shell's own, and `page.language.label` is the settings menu's. The
other twenty-two are the statistics panel's — a caption and an explanation for each of its eleven
`page.stats.*` tiles — written at runtime by `src/widgets/stats-panel/ui/stats-panel.ts` into the
region the shell leaves empty, and kept here because they are one prefix and read as one table. An
explanation is the body of the card its tile opens when pointed at or tabbed to, and not a `title`
attribute, which no keyboard can reach.

| Key                                 | English                                                                                                        | Notes                                                                                                                                                                 |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `page.title`                        | Elevator Saga - the elevator programming game, redesigned                                                      | twice: the `<title>`, and the `og:title` meta; worded apart from the original game's page, which the search engines have had for ten years                            |
| `page.description`                  | Elevator Saga, redesigned: write JavaScript to move people through a building efficiently. A modernized…       | twice: the `description` meta, and the `og:description` one                                                                                                           |
...
| `page.tagline`                      | A programming game: you write JavaScript that drives a building's elevators, and the passengers keep…          | the paragraph under that heading, and the only prose the shell carries; the app bar replaces the header it sits in, so nothing shows it once the bundle has run       |
...
| `page.stats.transportedTitle`       | How many passengers have reached the floor they asked for, so somebody still riding is not counted…            | the body of the card on the same tile as `page.stats.transported`                                                                                                     |
| `page.stats.elapsedTime`            | Elapsed time                                                                                                   |                                                                                                                                                                       |
| `page.stats.elapsedTimeTitle`       | The run's own clock, which the speed control makes pass faster or slower than real time, and which…            | the body of the card on the same tile as `page.stats.elapsedTime`; says whose clock it is, since the speed control moves it                                           |
| `page.stats.transportedPerSec`      | Transported per sec                                                                                            |                                                                                                                                                                       |
| `page.stats.transportedPerSecTitle` | Everyone delivered so far, over the time the run has taken, so it is the whole run's average rather…           | the body of the card on the same tile as `page.stats.transportedPerSec`; says the figure is the run's average, not the rate now                                       |
...
| `page.stats.avgWaitTimeTitle`       | The whole journey, from a passenger appearing in the building to stepping out at the floor they asked…         | the body of the card on the same tile as `page.stats.avgWaitTime`; what `docs.play.statistics.html` says of both delivery-time rows, said of one                      |
...
| `page.stats.avgPickupTimeTitle`     | The clock starts when a passenger appears and stops when a car takes them, and the row below it is the…        | the body of the card on the same tile as `page.stats.avgPickupTime`; text of `docs.play.statistics.html` word for word                                                |
...
| `page.stats.avgRideTimeTitle`       | The clock starts when a car takes a passenger and stops when they step out at their floor, so this and…        | the body of the card on the same tile as `page.stats.avgRideTime`; text of `docs.play.statistics.html` word for word                                                  |
| `page.stats.maxWaitTime`            | Max delivery time                                                                                              | likewise, and this is the figure the eight wait-limited levels are judged on                                                                                          |
| `page.stats.maxWaitTimeTitle`       | The longest any one passenger's whole journey has run, which keeps climbing while somebody is still…           | the body of the card on the same tile as `page.stats.maxWaitTime`; what that paragraph adds about the maximum alone                                                   |
| `page.stats.moves`                  | Moves                                                                                                          |                                                                                                                                                                       |
| `page.stats.movesTitle`             | One move is counted each time a car crosses the halfway mark between one floor and the next                    | the body of the card on the same tile as `page.stats.moves`                                                                                                           |
...
| `page.stats.stopsTitle`             | One stop is counted each time a car comes to rest at a floor and opens its doors, so a car sent to the…        | the body of the card on the same tile as `page.stats.stops`; text of `docs.play.statistics.html` word for word                                                        |
...
| `page.stats.peoplePerStopTitle`     | Everyone who got in or out, over the stops counted above, so opening the doors where nobody is waiting…        | the body of the card on the same tile as `page.stats.peoplePerStop`; text of `docs.play.statistics.html` word for word                                                |
...
| `page.stats.avgLoadTitle`           | How full the cars were, averaged over the moves counted above, so a car standing still is not in the…          | the body of the card on the same tile as `page.stats.avgLoad`; text of `docs.play.statistics.html` word for word                                                      |
...
### `documentation.html` — the reference page, 99 `docs.*` keys
...
it. It is also why that one key sits in `src/i18n/en.ts` while the other 98 sit in
`src/i18n/docs-en.ts`.

Those 98 are the page. `src/docs-page/render.ts` builds `documentation.html` out of them at build
...
| `docs.play.statistics.html`                         | Beside the building is a panel that keeps score while a run is going. Eight of its rows need a word. …          | markup                                                                                                                             |
...
| `docs.basics.declare.html`                          | Your code must declare a function called init. It can declare update alongside…                                 | markup                                                                                                                             |
| `docs.basics.example.code`                          | function init(elevators, floors) { // Do stuff with the elevators and floors, which are both arrays of ob…      | code; only the comments are translated; the one `docs.*` key with a call site — `src/ui/completions.ts` inserts it as the skeleton |
...
| `docs.basics.oldForm.html`                          | Solutions written for the original game declare an object instead, with init and up…                            | markup                                                                                                                             |
...
| Key                                 | English                                                                                                    | Notes                                                                                                                                              |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tutorial.levelN.startingCode.code` | function init(elevators, floors) { const elevator = elevators[0]; elevator.on("idle", function() { // TOD… | code; only the comments are translated; the program the editor is filled with, and the one `src/game/tutorial-solutions.test.ts` proves cannot win |
| `tutorial.levelN.solutionCode.code` | function init(elevators, floors) { const elevator = elevators[0]; elevator.on("idle", function() { elevat… | code; only the comments are translated; the answer, shown under the third hint and replayed as the fixture that must win                           |
...
| `tutorial.solution.copy`            | Copy this program                                                                                       | the name and the title of the icon button on the answer's code block, in `src/widgets/tutorial-panel/ui/tutorial-panel.ts` |
| `tutorial.solution.copied`          | Copied to your clipboard.                                                                               | the button's name and its `aria-live` line on a successful `navigator.clipboard.writeText`                                 |
| `tutorial.solution.copyFailed`      | Your browser refused to copy it. Select the code and copy it yourself.                                  | the same two, when the write refuses or the API is missing; the button wears a cross while it says so                      |
...
### `src/game/chapter2.ts` — 19 `chapter2.*` keys

Chapter two: levels built on how real lift systems are actually dispatched. One key per
...
Three levels carry a `title` and a `briefing.html` on top of that program, and that is the rule
rather than an accident: a card belongs to the level where a mechanic is met for the first time,
which is `level2` for traffic profiles, `level8` for zoning and `level11` for destination dispatch. The
card is the widest column on the screen, and a block that opened one on every level would spend it
restating what the level before had already explained — so the levels after a card are the idea
being asked for rather than described, and their region collapses to leave the building the width.
That is why `level2`'s paragraph ends by naming the evening peak and lunch traffic it does not itself
play: it is the only place levels 3 to 7 are introduced at all, exactly as `level8`'s is for 9 and 10
and `level11`'s is for 12 and 13.
...
| Key                                  | English                               | Notes                                                                                                                                                         |
| ------------------------------------ | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `chapter2.level1.startingCode.code`  | (the program the editor opens with)   | `.code`; only its `//` comment is translated, and `catalog.test.ts` holds the JavaScript byte-identical across locales                                        |
| `chapter2.level2.title`              | Everyone starts in the lobby          | the level's name, on the briefing card and nowhere else; the switcher's 118px trigger says "Level 2-2" instead                                                |
| `chapter2.level2.briefing.html`      | Ten floors, two cars, and a building… | markup; `<em>` around all three traffic profiles, since this card introduces them for levels 2 to 7                                                           |
| `chapter2.level2.startingCode.code`  | (the round-robin dispatcher)          | `.code`; the same program as `level1`'s apart from the `//` comment, which is the point — see below                                                           |
| `chapter2.level3.startingCode.code`  | (the sorted sweep)                    | `.code`; two `//` comment lines, and the only starter in the block a fixture also runs on other levels                                                        |
| `chapter2.level4.startingCode.code`  | (the round-robin dispatcher)          | `.code`; `level2`'s program with a `//` comment pointing at the `idle` handler instead                                                                        |
| `chapter2.level5.startingCode.code`  | (the sorted sweep)                    | `.code`; `level3`'s program unchanged, comments included                                                                                                      |
| `chapter2.level6.startingCode.code`  | (the round-robin dispatcher)          | `.code`; `level2`'s program with a `//` comment pointing at `callNextElevator`                                                                                |
| `chapter2.level7.startingCode.code`  | (the round-robin dispatcher)          | `.code`; `level2`'s program a fourth time, the `//` comment back on `floor_button_pressed`                                                                    |
| `chapter2.level8.title`              | Not every car goes everywhere         | the level's name, on the second of the block's three briefing cards                                                                                           |
| `chapter2.level8.briefing.html`      | Ten floors, and the two cars no long… | markup; `<em>` around _zones_ and `<code>` around `servedFloors()`, the method the mechanic is played through                                                 |
| `chapter2.level8.startingCode.code`  | (the sorted sweep)                    | `.code`; `level3`'s program with a `//` comment saying that not every car stops at every floor — it stalls this building                                      |
| `chapter2.level9.startingCode.code`  | (the zone-aware sweep)                | `.code`; `level3`'s program with `callNextElevator` filtering on `servedFloors()`, which is `level8`'s answer                                                 |
| `chapter2.level10.startingCode.code` | (the zone-aware sweep)                | `.code`; `level9`'s program, the `//` comment moved onto the three floors both banks serve                                                                    |
| `chapter2.level11.title`             | Nobody presses up or down             | the level's name, on the third and last briefing card of the block                                                                                            |
| `chapter2.level11.briefing.html`     | The hall buttons are gone. Instead o… | markup; `<em>` around _destination dispatch_, `<code>` around the event, the method that books a car, and the two ways a car is sent, the second in two calls |
| `chapter2.level11.startingCode.code` | (the booking rotation)                | `.code`; books a car for every journey and sends none, which is the mistake the card's last sentence names                                                    |
| `chapter2.level12.startingCode.code` | (the booking rotation that sends)     | `.code`; `level11`'s program with the sending added, the `//` comment moved onto whose turn it is                                                             |
| `chapter2.level13.startingCode.code` | (the nearest car with room)           | `.code`; the longest starter in the block — a distance-sorted queue and a load check — and both of its `//` comments run to two lines                         |

Four of the thirteen `.code` values are the same program with a different `//` comment, and three
more are the sorted sweep under three comments, and that is deliberate rather than a chance to
...
seed — `level3`'s and `level10`'s take silver, `level5`'s, `level9`'s, `level12`'s and `level13`'s bronze, and
the other seven lose. `src/game/chapter2.ts` records the figures that bracket each of them and
`src/game/chapter2-solutions.test.ts` re-runs them, so a wording change that reaches inside the
JavaScript fails a test rather than a level.

### `src/ui/templates.ts` — 13 `game.*` keys
...
| `game.seed.explanation`     | The same seed brings the same passengers, in the same order — played the same way, the run repeats exactly. | the field's `title`: a tooltip under the pointer, and the description a screen reader reads after the label  |
...
`t("game.seed.explanation")` inside `seedFieldTemplate`, which runs per render — which is the
...
### `src/widgets/stats-panel/ui/stats-panel.ts` — 5 `game.statsPanel.*` keys
...
ones `presentStats` and `goal-bar.ts`'s own meters already show, and their explanations with them —
so this prefix holds only the two figures with no production precedent, a caption and an
explanation apiece, plus the disclosure summary that reveals the panel's nine secondary tiles.
Every tile has one: the field carrying its key is required, so a fourteenth tile cannot arrive
unexplained.
...
| `game.statsPanel.waitingNowTitle` | How many passengers are standing on a floor at this moment with no car yet carrying them | the body of the card on the same tile as `game.statsPanel.waitingNow`                                      |
| `game.statsPanel.aboardNow`       | Riding now                                                                               | sums every elevator's occupied `userSlots`, the same test `elevatorSnapshot` uses                          |
| `game.statsPanel.aboardNowTitle`  | How many passengers are inside a car at this moment, on their way                        | the body of the card on the same tile as `game.statsPanel.aboardNow`                                       |
...
Mounted from `src/pages/game/index.ts`, which hands the panel the world and calls its `update` on a
language change.

### `src/widgets/level-switcher/ui/level-switcher.ts` — 11 `game.levelSwitcher.*` keys

The app bar's level-switcher popover, and its step buttons either side of the trigger. Each of its
two blocks of levels counts from one, so the bare digit a tile shows says nothing about which
chapter it belongs to: `game.levelSwitcher.levelTileLabel` puts the chapter back, both in the
accessible name of an unmedalled tile of either block and in the trigger, which stands under no
caption at all. The rest of the prefix is what a number cannot say: the block captions, the two
step buttons, the lesson tiles and their trigger, the sandbox tile, and the name a tile takes once
a medal is on record. The medal names themselves come from `game.goalBar.tier.*`, through
`TIER_NAME_KEY` in `src/entities/level-tier/ui/tier-name.ts`, which this widget and the goal bar
share so that one medal is called one word.
...
| `game.levelSwitcher.tutorialBlockLabel`      | Learning                         | the caption over the block of lessons; the panel around a lesson is named after the level it teaches, so this and the tiles under it are where the track is named at all       |
...
| `game.levelSwitcher.chapterBlockLabel`       | Chapter {number}                 | takes `{number}`; the caption over each block of levels, and the one place a chapter is named in words rather than in a tile's number                                          |
...
| `game.levelSwitcher.levelTileLabel`          | Level {chapter}-{number}         | takes `{chapter}` and `{number}`; accessible name of an unmedalled level tile in either chapter, and what the trigger reads while such a level is being played                 |
| `game.levelSwitcher.levelTileEarnedLabel`    | Level {chapter}-{number}, {tier} | takes `{chapter}`, `{number}` and `{tier}`; the same once a medal is on record. The badge's stars are `aria-hidden`, so this is the only place a screen reader hears the medal |
| `game.levelSwitcher.tutorialTileEarnedLabel` | Tutorial level {number}, {tier}  | takes `{number}` and `{tier}`; the same for a track tile, where `{tier}` is always gold                                                                                        |
...
The goal bar's own meters and its tier popover, above the building. `src/pages/game/index.ts`
mounts it with `presentGoalBar`, once per world, and calls the presenter's `update()` when the
language changes.
...
| `game.goalBar.tier.bronze`                  | Bronze                                                | a tier's display name; reached through `TIER_NAME_KEY` in `src/entities/level-tier/ui/tier-name.ts` by a popover row, by `trigger.titleEarned`'s `{tier}` and by the level switcher's two `...TileEarnedLabel` tiles                                            |
...
### `src/features/hotkeys-help/ui/hotkeys-modal.ts` — 20 `game.hotkeys.*` keys

The `hotkeys-help` feature's own dialog is `<dialog class="keys">`: a title, a close button and
three headed groups of rows pairing a hotkey with what it does. The split is not cosmetic — the
groups are the three scopes the shortcuts actually have, which is why each heading names its own
rather than leaving the rows to imply one. `Space` has a group to itself because the page claims it
only while nothing at all is focused, leaving the space bar to activate whatever is; the four under
it fire wherever the focus is, short of somewhere text is being typed; the rest are CodeMirror's
and reach nothing else. Mod- bindings are spelled as `<kbd>`s joined by `+`, the convention
`docs.play.shortcuts.html` already uses; `src/ui/shortcuts.ts`'s `labelModifierKeys` resolves each
one per visitor at runtime, so nothing here needs a static paragraph explaining that Windows and
Linux read `Ctrl` for `⌘`. Whoever mounts the dialog live still has to call `labelModifierKeys`
against it themselves, the same way `src/ui/localize-page.ts` already does for the rest of the page
shell.
...
| `game.hotkeys.nothingFocused`  | When nothing is focused                  | the first group's heading, over the one shortcut that asks for a bare page      |
| `game.hotkeys.startPause`      | Start and pause                          | the row naming `Space`, the whole of that group                                 |
| `game.hotkeys.outsideEditor`   | Outside the code editor                  | the second group's heading, over the rest of the page's own shortcuts           |
...
| `game.hotkeys.editorOnly`      | In the code editor                       | the third group's heading, over CodeMirror's own bindings                       |
| `game.hotkeys.applyCode`       | Apply the code and start over            | the row naming `Ctrl`+`Enter` a second time, for the editor's own binding       |
| `game.hotkeys.saveNow`         | Save right away                          | the row naming `Ctrl`+`S`                                                       |
| `game.hotkeys.completions`     | Suggest a call                           | the row naming `Ctrl`+`Space`, the one chord not marked `data-mod-key`          |
| `game.hotkeys.find`            | Find and replace                         | the row naming `Ctrl`+`F`                                                       |
| `game.hotkeys.findNext`        | Next match                               | the row naming `Ctrl`+`G`                                                       |
| `game.hotkeys.findPrevious`    | Previous match                           | the row naming `Ctrl`+`Shift`+`G`                                               |
| `game.hotkeys.selectNextMatch` | Add the next occurrence to the selection | the row naming `Ctrl`+`D`                                                       |
| `game.hotkeys.indent`          | Indent                                   | the row naming `Tab`                                                            |
| `game.hotkeys.leaveEditor`     | Move the focus out                       | the row naming `Esc`, which is how a keyboard leaves the editor at all          |
...
| `game.docs.intro.example.code`           | function init(elevators, floors) { // subscribe to events here } function update(dt, elevators, floo… | code; the skeleton every program starts from; comments translated only |
...
### `src/widgets/verdict-toast/ui/verdict-toast.ts` and `src/entities/level-tier/ui/tier-hint.ts` — 5 `game.feedback.*` keys

The rest of the end-of-run card: the medal it just awarded, the button that puts it away, and the
line under the message saying what the run would need for its next star. The four sentences above
are composed by the page and handed in; these five the card and the hint read for themselves,
which is the division `src/widgets/verdict-toast/ui/verdict-toast.ts` keeps everywhere — it is
told the verdict and works out none of it.

`game.feedback.tierEarned` is read out and not looked at. The card badges the tier with three
stars, those stars are sprite icons and every sprite icon is `aria-hidden`, and neither the title
nor the hint says which medal was won — the hint is about the next one. So the name goes in the
headline behind `.visually-hidden`, where a sighted player already has the stars.
...
| Key                              | English             | Notes                                                                                                                                                                      |
| -------------------------------- | ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `game.feedback.tierEarned`       | Level stars: {tier} | off-screen; takes `{tier}`, a `game.goalBar.tier.*` name, which a colon leaves nominative the way `game.goalBar.trigger.titleEarned` does                                  |
...
| `editor.defaultCode.code` | function init(elevators, floors) { const elevator = elevators[0]; // Let's use the first elevator // Wh… | code; only the comments are translated |
...
| `completion.global.skeleton`                   | Your code must declare a function called init. It can declare update alongside it, and any…                    | the entry whose `apply` is `docs.basics.example.code`                         |
...
| `completion.global.update`                     | Called repeatedly during the level, at a fixed rate of 100 times per game second. dt is always that fix…       |                                                                               |
| `completion.initSkeleton.code`                 | function init(elevators, floors) {                                                                             | code; only the comments are translated                                        |
| `completion.updateSkeleton.code`               | function update(dt, elevators, floors) {                                                                       | code; only the comments are translated                                        |
...
| `error.code.updateNotFunction`  | Code declares update as something other than a function                                                        | thrown, then shown in the code status bar; `update` itself is optional   |
...
| `src/game/fitness.ts`, `requireNothing`                                                                                        | `No requirement`                                                                                                                    | The benchmark's placeholder condition. Nothing draws a goal bar during a benchmark, so it never reaches a screen.                                                                                                                                        |
