...
а тесты уже воспроизводимо прогоняют целые задания с фиксированным сидом ГПСЧ. Повтору нужен сид _и_
...
5. **Рестарт по требованию, переключение уровней и управление скоростью времени** — в оригинале не
   было ничего из этого, кроме ссылки «Next challenge». У нас рестарт и скорость уже в панели
   задания (`src/ui/presenters.ts:188` и `challengeTemplate`), не хватает только прыжка на
   произвольный уровень — он есть у avanderw, скорость времени — у 2xh и в Cargo Dispatch.
...
10. **Признак «кабина полна»** — [curmudgeon22](https://news.ycombinator.com/item?id=33249988);
    ...
    `:637`) — до фасада не доведены только они.
