...
- **[avanderw/elevator-saga](https://github.com/avanderw/elevator-saga)**
  ...
  Диффы против 1.6.5 (сверено с нашим тегом `legacy-1.x`) маленькие и точечные:
  ...
  - **Быстрая навигация по заданиям** — ряд кнопок 1–18 плюс Demo прямо под полем задания; клик
    ...
    репозитории, и у нас её нет.
  - **Новая метрика: `avgLoadFactorOnMove`**, в панели статистики как «Avg load on move» в
    процентах. Реализована тремя правками: лифт на каждом ходе шлёт `elevator_moved` со своей
    загрузкой (`elevator.js`, +2 строки), мир копит сумму и делит на `moveCount` (`world.js`, +8),
    презентер печатает (`presenters.js`, +2). Это вторая ось оптимизации почти даром — та же идея,
  ...
  - **Редактор кода отключён.** `app.js` больше не создаёт CodeMirror, а `getCodeObj()` возвращает
    глобальный `system` из `ai/v3.js` — то есть страница исполняет решение автора (v1 7 КБ, v2 54
    КБ, v3 115 КБ), а не то, что вы напишете. Как игра это шаг назад, как стенд для одного алгоритма
    — ровно то, что нужно автору.
  - **Девятнадцать markdown-разборов** рядом с кодом: архитектура v1/v2/v3, дорожная карта, отчёты
    по тестам. Большая часть — про их алгоритм, но две записки полезны нам как факты о движке:
    `LOAD_FACTOR_TIMING.md` (когда именно `loadFactor()` успевает обновиться относительно
    `stopped_at_floor` — источник ложных «лишних остановок» в их логах) и `FULL_ELEVATOR_FIX.md`
    (загрузка почти никогда не равна 1.0, потому что пассажиры весят 55–100 кг, — поэтому у них
    порог 0.7).

  _(Прочитано: репозиторий, диффы против `legacy-1.x`, README и четыре разбора.)_
...
4. **Реалистичные профили трафика вместо равномерно случайных появлений** — joelthelion на HN, где
   **magwo подтвердил, что в коде есть неиспользованные распределения появления пассажиров**.
   ...
   интересен.
5. **Рестарт по требованию, переключение уровней и управление скоростью времени** — скорость есть у
   2xh и в Cargo Dispatch, прыжок на любое задание одной кнопкой — у avanderw. Оригинал даёт только
   ссылку «Next challenge», всё остальное — правка URL руками.
...
10. **Признак «кабина полна»** — [curmudgeon22](https://news.ycombinator.com/item?id=33249988);
    ...
    норме 100 на место, так что полная кабина показывает в среднем около 0.78 (это же выяснил и
    avanderw, поставив у себя порог 0.7). А самое дешёвое здесь то, что у нашего `Elevator` методы
    `isFull()` и `isEmpty()` уже есть внутри (`src/game/elevator.ts:623` и `:637`) — до фасада не
    доведены только они.
