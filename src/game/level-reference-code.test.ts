...
/** Confirms the two presets are real, loadable programs: the source parses, with callable `init` and `update`. */
describe.each([
 ...
 * the building's own level condition, so a livelocked run can be told apart
 * from one that legitimately lost by reading the raw counters directly.
 *
...
/**
 * Regression guard for a livelock where a full car stopped at a re-pressed
 * floor would splice that call ahead of an already-queued stop forever,
 * never reaching the queued floor or delivering anyone else again.
 */
 ...
 * level's own condition. Unlike `tutorial-solutions.test.ts`'s `playLevel`,
 * an undecided run returns `verdict: null` instead of throwing.
 *
 * @throws When the program throws.
 */
function playLevel(
...
/** Simulated seconds an undecided smoke-check run is allowed before it counts as a hang. */
const SMOKE_MAX_SIMULATED_SECONDS = 500;

/**
 * Broad termination smoke check: this dispatcher must always reach a decided
 * verdict, win or lose, never freeze. Only `verdict === null` is a failure.
 */
