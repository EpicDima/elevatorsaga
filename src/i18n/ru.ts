...
export const RU_MESSAGES: MessageCatalogue<"ru"> = {
  ...
  // Не «ждёт», а «доставка не длится»: ограничение в этих двух фразах — это
  ...
  //
  // На экране, впрочем, всегда оказывается только форма other. Оба места, где
  // собирается эта фраза (src/game/challenges.ts), передают сюда
...
elevator.goToFloor(2); // Всё равно добавится — очередь: 2, 3, 2`,
  ...
  "tutorial.task1.hint1.html":
    "Смотрите не в код, а на здание. Лифт стоит на нулевом этаже, и в очереди у него тот же нулевой этаж. Сколько всего этажей в этом доме?",
  ...
  "tutorial.task2.hint1.html":
    "Всё нужное было в первом задании: вы это видели, но не писали сами. Событие, которое случается, когда у лифта кончились цели, называется idle.",
  ...
  "tutorial.task4.hint2.html":
    'Очередь не пуста, но лифт о ней ничего не знает. После того как <span class="emphasis-color">destinationQueue</span> изменили вручную, игру надо об этом позвать, и нужный метод есть в списке методов лифта.',
  ...
  "tutorial.task6.goal":
    "Разберитесь, почему половина здания не садится в лифт, и перевезите 15 пассажиров так, чтобы доставка каждого не длилась дольше 28 секунд.",
