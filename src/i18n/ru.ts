...
function update(dt, elevators, floors) {
  ...
  "tutorial.level1.explanation.html":
    '<span class="emphasis-color">goToFloor</span> никуда не едет. Он дописывает этаж в конец <span class="emphasis-color">destinationQueue</span> и вызывает <span class="emphasis-color">checkDestinationQueue</span>, а дальше лифт разбирает очередь сам. Поэтому <span class="emphasis-color">goToFloor(0)</span>, когда кабина и так стоит на нулевом этаже, — это законная поездка нулевой длины: лифт приезжает туда, где стоит, открывает двери, люди заходят, очередь снова пуста, снова срабатывает <span class="emphasis-color">idle</span>, и снова происходит то же самое. Вот почему кабина наполняется, а счётчик перемещений держится на нуле. Пассажир садится в момент приезда и выходит на том этаже, который попросил, а этот лифт до него не доезжает. И ещё одно, о чём стоит сказать вслух: номер этажа за пределами дома — не ошибка, его молча приводят к ближайшему настоящему этажу. Тот, кто считает этажи с единицы, напишет здесь <span class="emphasis-color">goToFloor(2)</span> и тоже выиграет, потому что 2 превратится в 1.',
...
function update(dt, elevators, floors) {
  ...
  "tutorial.level2.explanation.html":
    '<span class="emphasis-color">init</span> вызывают один раз, на первом кадре прогона и до того, как мир сделает хоть один шаг, и обычно он только подписывается на события. Первое <span class="emphasis-color">idle</span> игра посылает сама, строкой сразу после того, как ваш <span class="emphasis-color">init</span> вернул управление, поэтому одной подписки хватает, чтобы всё завертелось. Вторая функция, <span class="emphasis-color">update(dt, elevators, floors)</span>, наоборот, вызывается на каждом шаге симуляции — 100 раз в игровую секунду. Дорожка ей ни разу не пользуется, и это намеренно: опрашивать состояние дома на каждом шаге — путь к программам похуже тех, которые просто отвечают на события. Похуже, но не запрещено: опросом проходится любой уровень дорожки.',
...
function update(dt, elevators, floors) {
  ...
  "tutorial.level3.explanation.html":
    'Пассажир, который зашёл в кабину, нажимает свой этаж, и игра сообщает об этом событием <span class="emphasis-color">floor_button_pressed</span>, передавая номер этажа аргументом. Горящие кнопки можно опрашивать и самому, через <span class="emphasis-color">getPressedFloors()</span>, но привычку стоит заводить другую: отвечать на событие. Заметьте, что <span class="emphasis-color">goToFloor(0)</span> в обработчике <span class="emphasis-color">idle</span> теперь никому не мешает — раз кнопки кабины обработаны, эта строка просто означает «вернуться на нулевой этаж, когда делать нечего».',
...
function update(dt, elevators, floors) {
  ...
  "tutorial.level4.explanation.html":
    'Стоящая полная кабина и стоящая пустая кабина отличаются так же, как лифт, который приехал и открыл двери, отличается от лифта, который не приехал ни разу. Посадка происходит в момент приезда, и больше нигде. Тот, кто нажал кнопку рядом со стоящей кабиной, обычно эту кабину и подталкивает: игра заново предлагает ей этот этаж вызовом <span class="emphasis-color">goToFloor(floor, true)</span>, и на учебных уровнях с первого по третий кабину наполняло именно это. Здесь толчок не делает ничего. Очередь не пуста, в ней 0, 1, 2, 3, а <span class="emphasis-color">goToFloor</span> отбрасывает просьбу, совпадающую с ближним концом непустой очереди, ещё до того, как дело дойдёт до проверки очереди: просят нулевой этаж, нулевой этаж и так первый в очереди, вызов возвращается. И кабина стоит так до конца прогона. <span class="emphasis-color">goToFloor</span> вызывает <span class="emphasis-color">checkDestinationQueue</span> за вас, а присваивание очереди — нет.',
...
function update(dt, elevators, floors) {
  ...
  "tutorial.level5.explanation.html":
    'Слепой объезд не масштабируется: в худшем случае человек ждёт целый круг, а круг растёт вместе с домом. Этажи умеют звать лифт сами. Оба события передают этаж аргументом, так что <span class="emphasis-color">floor.floorNum()</span> можно взять хоть из аргумента, хоть из замыкания — как читается лучше. Подписаться на оба события одной строкой тоже можно, <span class="emphasis-color">floor.on("up_button_pressed down_button_pressed", …)</span>, но тогда первым аргументом придёт имя сработавшего события, а этаж сдвинется на второе место; поэтому здесь два отдельных обработчика. И честно о результате: новая программа делает не меньше перемещений, чем объезд, а больше. Выигрывает она тем, что больше не возит воздух.',
...
function update(dt, elevators, floors) {
  ...
  "tutorial.level7.explanation.html":
    '<span class="emphasis-color">elevators[0]</span> — это не «лифт», это «первый лифт». В этом доме их два, а на последних уровнях игры их восемь. Программа, написанная через <span class="emphasis-color">elevators.forEach</span>, одинаково работает и с одной кабиной, и с восемью, и именно её вы унесёте на настоящие уровни. Выбирать по <span class="emphasis-color">loadFactor()</span> — самое дешёвое разумное правило: 0 — пусто, 1 — полно. Оно не единственное рабочее, годится что угодно, лишь бы обе кабины были при деле, но правило, которое сверяется с картинкой на экране, отлаживать легче.',
