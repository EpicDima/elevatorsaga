...
elevator.goingDownIndicator(false);`,
  ...
  // «С начала», а не «Заново»: рядом эти две кнопки, и одно слово на обеих
  // читалось бы как одна и та же. «Заново» остаётся у главной кнопки, когда
  // прогон уже закончился, — там выбора нет, только начать снова.
  //
  // «Код» в двух последних — потому что они одни в ряду действуют на редактор,
  // а не на симуляцию: «Сбросить» рядом с «С начала» прочли бы как ещё один
  // способ перезапустить прогон.
  "game.button.startOver": "С начала",
  ...
  // Пятая кнопка ряда и её подпись на время просчёта. Не «Быстро» и не
  // «Ускорить» — это уже делает регулятор скорости, только он всё равно
  // рисует здание, просто чаще. Здесь же на экране во время просчёта вообще
  // ничего не рисуется, поэтому и глагол другой. «Прогоняем…» вторит
  // «Считаем эффективность…» у fitness.measuring — тому же приёму: глагол в
  // настоящем времени и многоточие, единственная другая кнопка игры,
  // подменяющая свою подпись на время работы.
  ...
  "completion.global.update":
    "Вызывается многократно по ходу задания, с фиксированной частотой 100 раз в игровую секунду. dt всегда равен этому фиксированному шагу.",
  ...
  "docs.play.start.html":
    'Напишите код в окне под игровым полем и нажмите кнопку <span class="emphasis-color">Старт</span>, чтобы запустить его. Ничего применять заранее не нужно: программа сохраняется сама, пока вы печатаете, и каждый прогон читает её заново. Во время прогона та же кнопка читается как <span class="emphasis-color">Пауза</span>, а соседняя <span class="emphasis-color">С начала</span> обрывает прогон и начинает задание сначала с тем кодом, что к этому времени в редакторе.<br /> Скорость времени можно увеличивать и уменьшать кнопками {increase} и {decrease}.',
  ...
  "docs.play.shortcuts.html":
    'В редакторе <kbd data-mod-key>Ctrl</kbd>+<kbd>Enter</kbd> начинает задание заново с тем, что вы написали, — то же самое делает кнопка <span class="emphasis-color">С начала</span>; <kbd data-mod-key>Ctrl</kbd>+<kbd>S</kbd> записывает программу в хранилище сразу, не дожидаясь автосохранения, и не даёт открыться окну сохранения браузера; <kbd>Tab</kbd> добавляет отступ, а <kbd>Esc</kbd> убирает фокус из редактора.',
  ...
  "docs.basics.called.html":
    'Эти функции игра вызывает по ходу задания.<br /> <span class="emphasis-color">init</span> вызывается один раз, на первом кадре прогона, а не в момент, когда вы применили код; <span class="emphasis-color">update</span> — на том же шаге и на каждом следующем шаге симуляции — 100 раз в игровую секунду, по расписанию, привязанному к игровому времени, а не к тому, как часто рисует браузер. Поэтому <span class="emphasis-color">dt</span> всегда одно и то же значение, а два прогона с одним и тем же сидом и одинаковыми действиями проходят одну и ту же последовательность шагов — независимо от того, быстрый браузер или медленный. Обеим передаются одни и те же два массива — все лифты здания и все его этажи, — так что <span class="emphasis-color">elevators.length</span> и есть число кабин, которыми вы распоряжаетесь; между вызовами эти массивы не подменяются. Обе функции вызываются на объекте, который вы объявили, так что <span class="emphasis-color">this</span> внутри них — этот самый объект: всё, что программе нужно помнить от кадра к кадру, можно хранить на <span class="emphasis-color">this</span>, а не во внешней переменной. Это верно, пока они написаны через <span class="emphasis-color">function</span>: стрелочная функция сохраняет <span class="emphasis-color">this</span> того места, где её написали, а здесь это страница, а не ваш объект.',
...
elevator.goToFloor(2); // Всё равно добавится — очередь: 2, 3, 2`,
  ...
  "tutorial.task2.explanation.html":
    "init вызывают один раз, на первом кадре прогона и до того, как мир сделает хоть один шаг, и обычно он только подписывается на события. Первое idle игра посылает сама, строкой сразу после того, как ваш init вернул управление, поэтому одной подписки хватает, чтобы всё завертелось. Вторая функция, update(dt, elevators, floors), наоборот, вызывается на каждом шаге симуляции — 100 раз в игровую секунду. Дорожка ей ни разу не пользуется, и это намеренно: опрашивать состояние дома на каждом шаге — путь к программам похуже тех, которые просто отвечают на события. Похуже, но не запрещено: опросом проходится любое задание дорожки.",
