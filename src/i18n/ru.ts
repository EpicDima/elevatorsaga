...
export const RU_MESSAGES: MessageCatalogue<"ru"> = {
  ...
  "completion.elevator.isApproachingFloor":
    "Возвращает, едет ли лифт к указанному этажу и не проехал ли его. Учитывается только направление движения, так что лифт считается приближающимся и к тем этажам, что лежат дальше по ходу, даже если он остановится раньше.",
  ...
  "docs.basics.noLibraries.html":
    'Раньше игра подключала jQuery и lodash, поэтому в старых решениях с вики часто встречаются <span class="emphasis-color">$</span> и <span class="emphasis-color">_</span>. Ни та ни другая библиотека больше не подключается, и вашей программе они недоступны: решение с ними упадёт с ошибкой <span class="emphasis-color">_ is not defined</span>. Почти всё, ради чего они здесь были нужны, — <span class="emphasis-color">_.filter</span>, <span class="emphasis-color">_.map</span>, <span class="emphasis-color">_.each</span> и им подобные — есть у самих массивов (<span class="emphasis-color">filter</span>, <span class="emphasis-color">map</span>, <span class="emphasis-color">forEach</span>) в любом браузере, который потянет эту игру. Исключение — <span class="emphasis-color">_.min</span> и <span class="emphasis-color">_.max</span>: <span class="emphasis-color">Math.min</span> методом массива не является и принимает аргументы по одному, а не массивом, поэтому вместо <span class="emphasis-color">_.min(floorNums)</span> придётся писать <span class="emphasis-color">Math.min(...floorNums)</span>.',
...
elevator.goToFloor(2); // Всё равно добавится — очередь: 2, 3, 2`,
  ...
  "docs.api.elevator.isApproachingFloor":
    "Возвращает, едет ли лифт к указанному этажу и не проехал ли его. Учитывается только направление движения, так что лифт считается приближающимся и к тем этажам, что лежат дальше по ходу, даже если он остановится раньше. Это та же проверка, которую делает сама игра перед событием passing_floor, так что этаж, о котором эта проверка ответила «нет», это событие уже не вызовет. Стоящий лифт не приближается ни к чему, и тот, что уже приехал на нужный этаж, — тоже. Номер этажа за пределами здания подтягивается к ближайшему существующему этажу, а значение, которое числом не является вовсе (в том числе забытый аргумент), не принимается: игра сообщает об ошибке в вашем коде, как и в goToFloor.",
