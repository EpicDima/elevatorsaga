...
export const RU_MESSAGES: MessageCatalogue<"ru"> = {
  ...
  // Одно слово на оба состояния — как и в английском: что кнопка нажата,
  // сообщает `aria-pressed`, а не подпись. Повелительное наклонение, как у
  // четырёх соседок по ряду.
  "page.button.expand": "Развернуть",
  ...
  "completion.floor.event.hallButtonPressed":
    'Срабатывает, когда на этаже нажали любую из кнопок вызова. Обработчику передаётся направление вызова — "up" или "down" — и этаж, на котором нажали кнопку.',
...
elevator.goToFloor(2); // Всё равно добавится — очередь: 2, 3, 2`,
  ...
  "docs.api.floor.hallButtonPressed":
    'Срабатывает, когда на этаже нажали любую из кнопок вызова. Обработчику передаётся направление вызова — "up" или "down" — и этаж, на котором нажали кнопку. Приходит сразу после события up_button_pressed или down_button_pressed для того же нажатия, так что программа, подписанная на оба, узнает об этом нажатии дважды.',
