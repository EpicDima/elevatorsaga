/**
 * What every unit test file gets before it runs: every catalogue in memory.
 *
 * Named in `vite.config.ts` as the one Vitest setup file. It exists because
 * catalogues are fetched now rather than bundled — see the note at the top of
 ...
 * the catalogue. What covers that instead is explicit — `index.test.ts` and
 ...
 *
 * A setup file rather than an eager static import of the catalogues: an import
