...
export const EN_MESSAGES = {
  ...
  "page.description":
    ...
    "40 levels in two chapters and a learning track, a sandbox of your own size, and runs you can repeat.",
  "page.imageAlt":
  ...
  "page.tagline":
    ...
    "40 levels in two chapters and a learning track that starts from an empty editor, a sandbox " +
    "building of your own size, and runs you can repeat exactly.",
  ...
  "game.appBar.sourceCaption": "Code and license",
  "game.appBar.sourceForkLabel": "This game",
  "game.appBar.sourceOriginalLabel": "Original",
  ...
  "game.hotkeys.editorOnly": "In the code editor",
  "game.hotkeys.applyCode": "Apply the code and start over",
  "game.hotkeys.saveNow": "Save right away",
  "game.hotkeys.completions": "Suggest a call",
  "game.hotkeys.find": "Find and replace",
  "game.hotkeys.findNext": "Next match",
  "game.hotkeys.findPrevious": "Previous match",
  "game.hotkeys.selectNextMatch": "Add the next occurrence to the selection",
  "game.hotkeys.indent": "Indent",
  "game.hotkeys.leaveEditor": "Move the focus out",
...
elevator.goingDownIndicator(false);`,
  ...
  "editor.phrase.next": "next",
  "editor.phrase.previous": "previous",
  "editor.phrase.all": "all",
  "editor.phrase.matchCase": "match case",
  "editor.phrase.regexp": "regexp",
  "editor.phrase.byWord": "by word",
  "editor.phrase.replaceOne": "replace",
  "editor.phrase.replaceAll": "replace all",
  "editor.phrase.goToLine": "Go to line",
  "editor.phrase.go": "go",
  "editor.phrase.currentMatch": "current match",
  "editor.phrase.onLine": "on line",
  "editor.phrase.replacedMatches": "replaced $ matches",
  "editor.phrase.replacedOnLine": "replaced match on line $",
  "editor.phrase.close": "close",
  "editor.phrase.controlCharacter": "Control character",
  "editor.phrase.foldLine": "Fold line",
  "editor.phrase.unfoldLine": "Unfold line",
  "editor.phrase.foldedCode": "folded code",
  "editor.phrase.unfold": "unfold",
  "editor.phrase.foldedLines": "Folded lines",
  "editor.phrase.unfoldedLines": "Unfolded lines",
  "editor.phrase.to": "to",
...
function update(dt, elevators, floors) {
  ...
  "tutorial.level1.explanation.html":
    '<span class="emphasis-color">goToFloor</span> does not drive anywhere. It appends the floor to the end of <span class="emphasis-color">destinationQueue</span> and calls <span class="emphasis-color">checkDestinationQueue</span>, and the elevator works through that queue on its own. So <span class="emphasis-color">goToFloor(0)</span> while the car is already on floor 0 is a legal trip of zero length: the car arrives where it stands, opens its doors, people get in, the queue is empty again, <span class="emphasis-color">idle</span> fires again, and the same thing happens again.\n\nThat is why the car fills up while the moves counter stays at zero. A passenger boards on arrival and gets out on the floor they asked for, and this elevator never reaches it.\n\nOne more thing worth saying out loud: a floor number outside the building is not an error, it is quietly clamped to the nearest real floor. Somebody who counts floors from one writes <span class="emphasis-color">goToFloor(2)</span> here and wins as well, because 2 becomes 1.',
...
function update(dt, elevators, floors) {
  ...
  "tutorial.level4.explanation.html":
    'A full car standing still and an empty car standing still differ the way an elevator that arrived and opened its doors differs from an elevator that never arrived at all. Boarding happens on arrival, and nowhere else.\n\nSomeone who presses a button beside a standing car usually nudges it: the game re-offers the floor to the car with <span class="emphasis-color">goToFloor(floor, true)</span>, and on tutorial levels 1 to 3 that is what kept filling the cabin. Here the nudge does nothing.\n\nThe queue is not empty, it holds 0, 1, 2, 3, and <span class="emphasis-color">goToFloor</span> drops a request that equals the adjacent end of a non-empty queue before it ever gets as far as checking the queue: floor 0 is asked for, floor 0 is already at the head, and the call returns. The car stands there for the rest of the run. <span class="emphasis-color">goToFloor</span> calls <span class="emphasis-color">checkDestinationQueue</span> for you; assigning the queue does not.',
...
function update(dt, elevators, floors) {
  ...
  "tutorial.level5.explanation.html":
    'A blind sweep does not scale: its worst waiting time is the length of one lap, and the lap grows with the building. Floors can call an elevator themselves.\n\nBoth events hand the floor over as the argument, so <span class="emphasis-color">floor.floorNum()</span> can come from the argument or from the closure, whichever reads better. Subscribing to both events in one line is possible — <span class="emphasis-color">floor.on("up_button_pressed down_button_pressed", …)</span> — but then the first argument is the name of the event that fired and the floor moves along into second place; that is why there are two separate handlers here.\n\nAnd to be honest about the result: the new program makes more moves than the sweep did, not fewer. It wins by no longer carrying air.',
...
function update(dt, elevators, floors) {
  ...
  "tutorial.level8.hint3.html":
    'The answer is the program from tutorial level 7, unchanged — it works just as well with one elevator. Subscribe to <span class="emphasis-color">floor_button_pressed</span> on every car, subscribe to both call buttons on every floor, and send a car to <span class="emphasis-color">floor.floorNum()</span>.\n\nWrite the whole of it: the half where a car simply parks at floor 0 and knows nothing but its own cabin buttons is the half that loses runs.',
  "tutorial.level8.explanation.html":
    "Nothing here is new, and that is the point. This is the building of level 1 and the bar of level 1, copied deliberately: three floors, one elevator, 15 passengers in 60 seconds. Win here and level 1 is already solved, by the very program now in the editor.\n\nThe margin here is also the thinnest on the track, and that is not the track's doing: at 0.3 passengers a second the fifteenth passenger does not appear in the building until about the forty-seventh second of the sixty, so the minute is tighter than it looks. That is a property of level 1, and you have met it early.",
