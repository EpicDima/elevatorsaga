...
export const EN_DOCS_MESSAGES = {
  ...
  "docs.basics.oldForm.html":
    'Solutions written for the original game declare an object instead, with <span class="emphasis-color">init</span> and <span class="emphasis-color">update</span> as its members and a pair of braces around the lot. Those still run here exactly as they are — paste one in and press <span class="emphasis-color">Start over</span>. In that form the two are called as methods of the object you wrote, so <span class="emphasis-color">this</span> inside them is that object, and that is where such a solution usually keeps what it remembers. It works as long as they are written with <span class="emphasis-color">function</span>: an arrow function keeps the <span class="emphasis-color">this</span> of wherever it was written, which here is the page rather than your object.',
