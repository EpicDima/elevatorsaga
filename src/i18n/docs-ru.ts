...
export const RU_DOCS_MESSAGES: DocsCatalog = {
  ...
  "docs.play.track.html":
    'Если вы никогда не писали таких программ, начните с <a href="index.html#level=tutorial-1">учебной дорожки</a> — в меню уровней наверху страницы её уроки собраны под заголовком <span class="emphasis-color">Обучение</span>. Это восемь небольших зданий, которые знакомят с этим API по одной ошибке за раз: в каждом выдаётся программа, которая проигрывает, и нужно найти в ней единственную ошибку — рядом есть подсказки и разбор того, что на самом деле происходило в прогоне.',
  ...
  "docs.basics.oldForm.html":
    'Решения, написанные для оригинальной игры, объявляют вместо этого объект: <span class="emphasis-color">init</span> и <span class="emphasis-color">update</span> — его поля, а вокруг всего стоит пара фигурных скобок. Такие решения работают здесь как есть — вставьте и нажмите <span class="emphasis-color">Начать заново</span>. В этой форме обе функции вызываются как методы объявленного вами объекта, так что <span class="emphasis-color">this</span> внутри них — этот самый объект, и обычно такое решение хранит на нём всё, что помнит. Это верно, пока они написаны через <span class="emphasis-color">function</span>: стрелочная функция сохраняет <span class="emphasis-color">this</span> того места, где её написали, а здесь это страница, а не ваш объект.',
