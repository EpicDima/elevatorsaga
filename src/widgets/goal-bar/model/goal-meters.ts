/**
 * The main goal bar's own meters — one per figure a challenge's bronze
 ...
 * `ChallengeCondition`'s own {@link TierRequirementInfo} array, so a caller
 * passes `challenge.condition.requirements` straight through rather than this
 * module reaching into `entities/challenge` itself for it.
 *
...
import { requirementMet, requirementProgress } from "#entities/challenge-tier/index.ts";
 ...
 * `entities/challenge`/`ChallengeCondition.requirements` list the delivery
 ...
 *
 * @param requirements - A challenge's own requirement list, e.g. its
